...
Требование: Java 21 (запускайте Gradle с `JAVA_HOME` на JDK 21, иначе возможна ошибка `Unsupported class file major version 69`).

Локальный запуск:
```
JAVA_HOME=/Users/nikitazubehin/Library/Java/JavaVirtualMachines/ms-21.0.9/Contents/Home ./gradlew clean ui_test -Denv=local
```

Отчет Allure после запуска:
```
JAVA_HOME=/Users/nikitazubehin/Library/Java/JavaVirtualMachines/ms-21.0.9/Contents/Home ./gradlew allureReport
```
Отчет будет сформирован в `build/reports/allure-report`.
